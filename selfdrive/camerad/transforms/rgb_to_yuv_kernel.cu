...
__device__ void convert_2_ys(uint8_t * out_yuv, int yi, const uint8_t *rgbs1) {
  uint8_t yy[2] = {
...
__device__ void convert_4_ys(uint8_t * out_yuv, int yi, const uint8_t *rgbs1, const uint8_t *rgbs3) {
  const uint8_t yy[4] = {
    (uint8_t) RGB_TO_Y(rgbs1[2], rgbs1[1], rgbs1[0]),
    (uint8_t) RGB_TO_Y(rgbs1[5], rgbs1[4], rgbs1[3]),
...
__device__ void convert_uv(uint8_t * out_yuv, int ui, int vi,
                    const uint8_t * rgbs1, const uint8_t * rgbs2) {
...
__device__ void convert_2_uvs(uint8_t * out_yuv, int ui, int vi,
                    const uint8_t *rgbs1, const uint8_t *rgbs2, const uint8_t *rgbs3, const uint8_t *rgbs4) {
  // U & V: average of 2x2 pixels square
...
__global__ void rgb_to_yuv(uint8_t const * const rgb, uint8_t * out_yuv, int width, int height, int rgb_stride)
{
  ...
  const int dy = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  uint8_t *rgbs0_0 = (uint8_t *) malloc(8); 
  uint8_t *rgbs0_1 = (uint8_t *) malloc(8);
  uint8_t *rgbs1_0 = (uint8_t *) malloc(8);
  uint8_t *rgbs1_1 = (uint8_t *) malloc(8);
  uint8_t *rgbs2_0 = (uint8_t *) malloc(8);
  uint8_t *rgbs2_1 = (uint8_t *) malloc(8);
  uint8_t *rgbs3_0 = (uint8_t *) malloc(8);
  uint8_t *rgbs3_1 = (uint8_t *) malloc(8);
...
void start_kernel(size_t *work_size, void *rgb_cuda, void *yuv_cuda, int width, int heigth, int rgb_stride) {
  dim3 grid = {(uint)work_size[0] / 10, (uint)work_size[1] / 10};
  dim3 block = {10,10,1};
