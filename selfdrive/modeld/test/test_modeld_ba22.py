...
VIPC_STREAM = {"roadCameraState": VisionStreamType.VISION_STREAM_ROAD}
...
class TestModeld(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.pm = messaging.PubMaster(['roadCameraState', 'liveCalibration', 'lateralPlan'])
  ...
  def test_dropped_frames(self):
    ...
    for n in range(1, 50):
      if (random.random() < 0.1) and n > 3:
        cams = random.choice([(), ('roadCameraState', )])
